 ...
 *
 * Categorizes errors into actionable kinds so the circuit breaker and
 * retry logic can make smarter decisions. Auth errors should not trip
 * the circuit breaker (they are not transient). Offline and timeout
 * errors are transient and should count toward the breaker threshold.
 */
...
/**
 * Analyze a connection error and return a classified issue.
 */
...
/**
 * Check if an issue is an auth problem (should not trip the circuit breaker).
 */
...
function extractStatusCode(message: string): number | undefined {
  ...
  // Try parsing JSON error bodies
  const trimmed = message.trim();
  if (trimmed.startsWith('{')) {
    try {
      const obj = JSON.parse(trimmed);
      const val = findStatusInObject(obj);
...
function extractStdioExit(message: string): { stdioExitCode?: number; stdioSignal?: string } | undefined {
  ...
  if (!exitMatch && !signalMatch) return undefined;
...
function findStatusInObject(value: unknown): unknown {
  ...
  if (typeof record.status === 'number' || typeof record.status === 'string') return record.status;
  if (typeof record.code === 'number' || typeof record.code === 'string') return record.code;
  if (typeof record.error === 'object' && record.error !== null) return findStatusInObject(record.error);
  return undefined;
}
